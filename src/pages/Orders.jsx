...
export default function Orders() {
  ...
  async function save() {
    if (editing && isDelivered(editing.status)) return
  ...
  return (
    <Box className="cake-page">
      ...
      <Paper sx={{ overflow: 'auto', borderRadius: 3, border: '1px solid var(--cake-border)', boxShadow: '0 12px 24px rgba(61,31,14,0.04)' }}>
        <Box className="mobile-records">
          {filteredOrders.map(row => {
            const delivered = isDelivered(row.status)
            return (
              <Box className="mobile-record" key={row.order_id}>
                ...
                <Box className="mobile-record-actions">
                  <Button startIcon={delivered ? <Visibility /> : <Edit />} onClick={() => openView(row)}>{delivered ? 'View order' : 'Edit order'}</Button>
                  {!delivered && <IconButton aria-label="Delete order" color="error" onClick={() => remove(row)}><Delete /></IconButton>}
                </Box>
              </Box>
            )
          })}
              ...
              return (
                <TableRow key={row.order_id} hover sx={{ '&:last-child td': { borderBottom: 0 } }}>
                  ...
                  <TableCell align="right">
                    <IconButton onClick={() => openView(row)} sx={{ color: 'var(--choco-muted)' }}>{delivered ? <Visibility /> : <Edit />}</IconButton>
                    {!delivered && <IconButton color="error" onClick={() => remove(row)}><Delete /></IconButton>}
                  </TableCell>
                </TableRow>
              )
            })}
      ...
      <Dialog open={open} onClose={() => setOpen(false)} fullWidth maxWidth="sm">
        <DialogTitle sx={{ fontWeight: 800, color: 'var(--ink)', pb: 1 }}>{editing && isDelivered(editing.status) ? 'Order Details' : editing ? 'Edit Cake Order' : 'New Cake Order'}</DialogTitle>
        <DialogContent>
          <Box display="grid" gap={2} pt={1}>
            {error && <Alert severity="error" sx={{ borderRadius: 2 }}>{error}</Alert>}
            <TextField label="Cake Name" value={form.cake_name} onChange={e => setForm({ ...form, cake_name: e.target.value })} required sx={inputStyle} InputProps={{ readOnly: !!editing && isDelivered(editing.status) }} />
            <TextField label="Customer Name" value={form.customer_name} onChange={e => setForm({ ...form, customer_name: e.target.value })} required sx={inputStyle} InputProps={{ readOnly: !!editing && isDelivered(editing.status) }} />
            <TextField label="Mobile Number" value={form.mobile_number} onChange={e => setForm({ ...form, mobile_number: e.target.value })} required sx={inputStyle} InputProps={{ readOnly: !!editing && isDelivered(editing.status) }} />
            <TextField label="Order Date" type="date" value={form.order_date} onChange={e => setForm({ ...form, order_date: e.target.value })} slotProps={{ inputLabel: { shrink: true } }} required sx={inputStyle} InputProps={{ readOnly: !!editing && isDelivered(editing.status) }} />
            <TextField label="Delivery Date" type="date" value={form.delivery_date} onChange={e => setForm({ ...form, delivery_date: e.target.value })} slotProps={{ inputLabel: { shrink: true } }} required sx={inputStyle} InputProps={{ readOnly: !!editing && isDelivered(editing.status) }} />
            <TextField label="Price" type="number" inputProps={{ min: 0, step: '0.01', readOnly: !!editing && isDelivered(editing.status) }} value={form.price} onChange={e => setForm({ ...form, price: e.target.value })} required sx={inputStyle} />
            <TextField label="Remarks" multiline minRows={2} value={form.remarks} onChange={e => setForm({ ...form, remarks: e.target.value })} sx={inputStyle} InputProps={{ readOnly: !!editing && isDelivered(editing.status) }} />
          </Box>
        </DialogContent>
        <DialogActions sx={{ px: 3, pb: 2 }}>
          <Button onClick={() => setOpen(false)} sx={{ color: 'var(--choco)', textTransform: 'none', fontWeight: 600 }}>Cancel</Button>
          {!editing || !isDelivered(editing.status) ? (
